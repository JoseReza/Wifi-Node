...
void setup() {
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest * request) {
    ...
    if (request->hasParam("lowvelocity")) {
      String inputMessage = request->getParam("lowvelocity")->value();
      if(inputMessage == "On"){digitalWrite(lowVelocityPin, HIGH);}
      else if(inputMessage == "Off"){digitalWrite(lowVelocityPin, LOW);};
    }
    if (request->hasParam("mediumvelocity")) {
      String inputMessage = request->getParam("mediumvelocity")->value();
      if(inputMessage == "On"){digitalWrite(mediumVelocityPin, HIGH);}
      else if(inputMessage == "Off"){digitalWrite(mediumVelocityPin, LOW);};
    }
    if (request->hasParam("highvelocity")) {
      String inputMessage = request->getParam("highvelocity")->value();
      if(inputMessage == "On"){digitalWrite(highVelocityPin, HIGH);}
      else if(inputMessage == "Off"){digitalWrite(highVelocityPin, LOW);};
    }
    if (request->hasParam("waterpump")) {
      String inputMessage = request->getParam("waterpump")->value();
      if(inputMessage == "On"){digitalWrite(waterPumpPin, HIGH);}
      else if(inputMessage == "Off"){digitalWrite(waterPumpPin, LOW);};
    }
  });
